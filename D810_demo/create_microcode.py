...
class microcode_viewer_t(kw.simplecustviewer_t):
    ...
    def OnKeydown(self, vkey, shift):   #响应键盘事件  快捷键
        if vkey == ord("G"):
            self.codeFlow()
        if vkey == ord("A"):
            self.mba.copy_block(self.mba.get_mblock(1), self.mba.qty - 1)
            self.mba.copy_block(self.mba.get_mblock(1), self.mba.qty - 1)
            # print("  Duplicated {0} ".format( duplicated_blk.serial))
            self.codeFlow()

        if vkey == ord("C"):
            dup_block1 = self.mba.copy_block(self.mba.get_mblock(1), self.mba.qty - 1)
            insn = dup_block1.head
            ...
            #将签名创建的insn 插入到 dup_block1.tail
            dup_block1.insert_into_block(insn, dup_block1.head)

            self.codeFlow()

        if vkey == ord("D"):
            dup_block1 = self.mba.copy_block(self.mba.get_mblock(1), self.mba.qty - 1)
            insn = ida_hexrays.minsn_t(dup_block1.head)
            insn.opcode = ida_hexrays.m_add  # 加法操作
            insn.l = ida_hexrays.mop_t(ida_hexrays.mop_r, 8)  # 左操作数
            insn.r = ida_hexrays.mop_t(64, 8)  # 右操作数
            insn.d = ida_hexrays.mop_t(32, 8)  # 右操作数
            ...
            #将签名创建的insn 插入到 dup_block1.tail
            dup_block1.insert_into_block(insn, dup_block1.head.next)
