...
class MicroBlockText(TextBlock):
    ...
    def _generate_header_lines(self):
        ...
        shape_tokens = [TextCell("[START="), AddressToken(blk.start), TextCell(" END="), AddressToken(blk.end),
                        TextCell("] "),
                        TextCell("STK=%X/ARG=%X, MAXBSP: %X" % (blk.minbstkref, blk.minbargref, blk.maxbsp))]

        # assemble the 'main' block header line
        all_tokens = [TextCell("%s-BLOCK " % blk_type), BlockNumberToken(blk.serial),
                      TextCell(" ")] + prop_tokens + shape_tokens
