...
class UnflattenerFakeJump(GenericDispatcherUnflatteningRule):
    ...
    def func(self, blk: mblock_t):
        ...
        if blk.mba.maturity != hr.MMAT_GLBOPT2:
            return 0
        if True == self.set_dispatch_block(25,blk):
            # now = datetime.now()
        ...
        if True == self.set_dispatch_block(3,blk):
            self.last_pass_nb_patch_done = self.remove_flattening()
            print("Unflattening at maturity {0} pass {1}: {2} changes".format(self.cur_maturity, self.cur_maturity_pass,
